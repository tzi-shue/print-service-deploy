...
function getPrinters(): array
{
    ...
    if ($win) {
        ...
        foreach ($out as $l) {
            if (strpos($l, 'Name=') === 0) $list[] = trim(substr($l, 5));
        }
        exec('wmic printer where "Default=\'TRUE\'" get name /format:list', $def);
        foreach ($def as $l) {
            if (strpos($l, 'Name=') === 0) {
                $default = trim(substr($l, 5));
                break;
            }
        }
    } else {
        exec('lpstat -a 2>&1', $pa);
        foreach ($pa as $l) if (preg_match('/^(\S+)\s+accept/i', $l, $m)) $list[] = $m[1];
        exec('lpstat -d 2>&1', $d);
        if (preg_match('/system default destination:\s*(\S+)/', implode('', $d), $m)) $default = $m[1];
    }
    return [$list ?? [], $default];
}
...
.qr-box img{border:1px solid #ddd; border-radius:4px}
.upload-area{margin-top:30px; text-align:center}
...
if ($win) {
    exec('wmic printer where "Default=\'TRUE\'" get name /format:list', $def);
    foreach ($def as $l) if (strpos($l, 'Name=') === 0) $default = trim(substr($l, 5));
} else {
    exec('lpstat -d 2>&1', $d);
    if (preg_match('/system default destination:\s*(\S+)/', implode('', $d), $m)) $default = $m[1];
}
...
/* 按类型打印（保持你原有逻辑，仅linux示例） */
$success = false; $output = []; $method = '';
try {
    $ext = strtolower($file_ext);
    if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
        $method = 'image';
        $cmd    = "lp -d " . escapeshellarg($target) . " " . escapeshellarg($tmpFile) . " 2>&1";
        exec($cmd, $output, $ret);
        $success = ($ret === 0);
