...
install_base_tools() {
    ...
    if type apt-get >/dev/null 2>&1; then
        export DEBIAN_FRONTEND=noninteractive
        apt-get install -y --no-install-recommends wget curl qrencode jq || {
            clean_cache
            apt-get install -y --no-install-recommends wget curl qrencode jq || error_exit "安装工具失败"
        }
    else
        yum install -y wget curl qrencode jq || error_exit "安装工具失败"
    fi
}
