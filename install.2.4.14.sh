...
install_cups() {
    ...
    if command -v cupsd &> /dev/null || command -v lpstat &> /dev/null; then
        CUPS_VERSION=$(cupsd --version 2>/dev/null | grep -oP '\d+\.\d+\.\d+' || lpstat -v 2>&1 | grep -oP 'CUPS v\K[0-9.]+' || echo "未知版本")
        ...
        apt-get install -y build-essential libavahi-client-dev libgnutls28-dev \
            libpam0g-dev zlib1g-dev libsystemd-dev libavahi-common-dev \
            libavahi-client-dev libgnutls28-dev libpam0g-dev zlib1g-dev \
            libusb-1.0-0-dev libdbus-1-dev libpaper-dev libkrb5-dev \
            libavahi-compat-libdnssd-dev || {
            print_error "编译依赖安装失败"
            exit 1
        }
        ...
        cd /tmp
        if ! wget -O cups-${CUPS_VERSION}-source.tar.gz "$CUPS_URL"; then
            print_error "CUPS 源码下载失败"
            exit 1
        fi
        ...
        ./configure --prefix=/usr \
            ...
            --with-cups-user=lp \
            --with-cups-group=lp || {
            print_error "CUPS 配置失败"
            exit 1
        }
        
        print_msg "编译 CUPS (这可能需要几分钟)..."
        make -j$(nproc) || {
            print_error "CUPS 编译失败"
            exit 1
        }
    ...
    print_msg "CUPS 状态: $(systemctl is-active cups 2>/dev/null || echo '未运行')"
}
